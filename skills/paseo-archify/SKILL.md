...
## Paseo delivery contract

Inside Paseo, do not run the Archify CLI directly and do not write HTML into the repository. Author the JSON specification, then call the Paseo `archify_render` tool with the diagram type and the complete specification. That tool validates with the showcase profile, writes the artifact atomically under `$PASEO_HOME/archify/<workspaceId>/`, and returns the artifact receipt. If validation fails, repair the specification from the returned diagnostics and call the tool again.
Skip the update-awareness check; Paseo owns the pinned vendored runtime.
